...
def get_dataloaders(args):
    ...
    if dataset_name == 'sphere':
        ...
        full_train_dataset = datasets.CIFAR10(root='~/datasets', train=True, transform=transform, download=False)
        test_dataset = datasets.CIFAR10(root='~/datasets', train=False, transform=transform, download=False)
        ...
        transform = transforms.Compose([
            transforms.Resize((args.image_size, args.image_size)),  # Add resizing if needed
            transforms.ToTensor(),
            transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
        ])

        # Download CelebA dataset (requires login for first download)
        full_dataset = datasets.CelebA(root='~/datasets', split='train', transform=transform, download=True)
        test_dataset = datasets.CelebA(root='~/datasets', split='test', transform=transform, download=True)
