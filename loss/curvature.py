...
def _solve_upper_tri_batch(U: Tensor, B: Tensor) -> Tensor:
    """
    ...
    try:
        ...
        X, _ = torch.triangular_solve(B, U, upper=True)
    return X.squeeze(-1) if add_dim else X
...
def _II_in_normal_dir_batched(
    ...
    if use_exact_hessian:
        ...
        def H_for_normal(ai: Tensor, zi: Tensor) -> Tensor:
            eps = fd_eps
            def jv(zz, vv): return jvp(f_single, (zz,), (vv,))[1]
            Jv_pos = vmap(jv, in_dims=(None, 0))(zi + eps * I, I)    # (m,D)
            Jv_now = vmap(jv, in_dims=(None, 0))(zi, I)
