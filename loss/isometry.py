...
def _qr_frame(B: int, dim: int, r: int, *, device, dtype) -> torch.Tensor:
    """
    Returns r orthonormal vectors (per sample) in R^dim.
    Output shape: (r, B, dim). Uses QR for numerical stability.
    """
    r = min(r, dim)
...
def encoder_isometry_regularisation(
    ...
    *,
    ...
    # Euclidean orthonormal frame in latent
    U = _qr_frame(B, k, r, device=device, dtype=y.dtype)  # (r, B, k)
...
def decoder_isometry_regularisation(
    ...
    *,
    ...
    """
    assert z.dim() == 2, "decoder_isometry_regularisation expects z of shape (B, d)"
    B, d = z.shape
    z = z.contiguous().requires_grad_(True)

    # build orthonormal u_i in latent (Euclidean)
    U = _qr_frame(B, d, min(num_v, d), device=device, dtype=z.dtype)  # (r,B,d)
    U = U.reshape(U.shape[0], B, *z.shape[1:])  # match shape for decode JVPs

    def jvp_single(u_single):  # (B,d) -> (B, *xshape)
