...
def get_loss_fn(config, sde, t_dist):
    ...
    guidance_interval = config.training.get('guidance_interval', (1e-2, 0.8))
    guidance_weight = config.training.get('guidance_weight', 1.4)
    def loss_fn(model, batch, train):
        encoder = model.encoder
        noise_predictor_fn = get_noise_fn(sde, model, train, G=guidance_weight, guidance_interval=guidance_interval)
        x, y = batch
...
def get_old_loss_fn(config, sde, t_dist):
    ...
    def loss_fn(model, batch, train):
        encoder = model.encoder
        score_fn = get_score_fn(sde, model, train, G=guidance_weight, guidance_interval=guidance_interval)
        x, y = batch
