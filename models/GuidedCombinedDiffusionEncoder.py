...
class GuidedCombinedDiffusionEncoder(nn.Module):
    def __init__(self, config):
        super(GuidedCombinedDiffusionEncoder, self).__init__()
    ...
    def prepare_diffusion_model(self, config):
        ...
        if 'pytorch-lightning_version' in checkpoint.keys():
            ...
            self.diffusion_model.load_state_dict(new_state_dict, strict=True)
        else:
    ...
    def get_noise_predictor_fn(self, sde, train=False, G=1.0, guidance_interval=None):
        """
        ...
        def noise_predictor_fn(x, cond, t):
            ...
            if (guidance_interval is not None) and (t is not None) and (not train):
                ...
                return pretrained_noise_prediction + latent_correction_noise
        
        return noise_predictor_fn


    def get_score_fn(self, sde, train=False, G=None, guidance_interval=None):
        """
        ...
        def score_fn(x, cond, t):
            ...
            # Calculate the latent correction noise
            latent_score = latent_correction_fn(x, z, t)
