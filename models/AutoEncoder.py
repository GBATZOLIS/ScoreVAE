...
class ConvEncoder(nn.Module):
    ...
    Convolutional encoder with:
      ...
      • mu/logvar heads for VAE (used by AutoEncoder when enabled)
    """
...
class AutoEncoder(nn.Module):
    """
    Minimal AE/VAE with encode/decode methods for Jacobian regularization.

    Backward-compat:
      • When VAE disabled: identical to your original AE.
    ...
    def __init__(self, cfg):
        ...
        #For normalization of the latent space
    ...
    def normalize_latent(self, z: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
        """ẑ = (z - μ) / σ per-dimension; safe with broadcasting."""
    ...
    def sample_z(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # (optional) clamp for numerical stability under AMP
