# models/autoencoder_coordconv.py
from __future__ import annotations
import math
from typing import Tuple

import torch
import torch.nn as nn


# ─────────────────────────── utils ─────────────────────────── #

class SimpleGroupNorm2d(nn.Module):
    """
    GroupNorm with per-channel affine. Construct via _gn_for to guarantee divisibility.
    """
    def __init__(self, num_groups: int, num_channels: int, eps: float = 1e-5, affine: bool = True):
        super().__init__()
        assert num_channels % num_groups == 0, "channels must be divisible by num_groups"
        self.num_groups, self.num_channels, self.eps, self.affine = num_groups, num_channels, eps, affine
        if affine:
            self.weight = nn.Parameter(torch.ones(1, num_channels, 1, 1))
            self.bias = nn.Parameter(torch.zeros(1, num_channels, 1, 1))
        else:
            self.register_parameter("weight", None)
            self.register_parameter("bias", None)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        N, C, H, W = x.shape
        G = self.num_groups
        xg = x.reshape(N, G, C // G, H, W)
        mean = xg.mean(dim=(2, 3, 4), keepdim=True)
        var = xg.var(dim=(2, 3, 4), keepdim=True, unbiased=False)
        xg = (xg - mean) / torch.sqrt(var + self.eps)
        y = xg.reshape(N, C, H, W)
        if self.affine:
            y = y * self.weight + self.bias
        return y


def _choose_gn_groups(C: int, preferred: int) -> int:
    """
    Robust group chooser: enforces divisibility; falls back to gcd; clamps to [1, C].
    """
    g = min(preferred, C)
    g = math.gcd(g, C)
    if g == 0:
        g = 1
    return max(1, min(g, C))


def _gn_for(C: int, preferred_groups: int) -> SimpleGroupNorm2d:
    return SimpleGroupNorm2d(_choose_gn_groups(C, preferred_groups), C)


def _make_grid(H: int, W: int, dtype: torch.dtype = torch.float32) -> torch.Tensor:
    """
    Build a (2, H, W) grid with x,y in [-1, 1]. Registered as buffers; forward casts dtype.
    """
    yy, xx = torch.meshgrid(
        torch.linspace(-1.0, 1.0, H, dtype=dtype),
        torch.linspace(-1.0, 1.0, W, dtype=dtype),
        indexing="ij",
    )
    return torch.stack([xx, yy], dim=0).contiguous()


def _bilinear_kernel_2d(k: int) -> torch.Tensor:
    factor = (k + 1) // 2
    center = factor - 1 if k % 2 == 1 else factor - 0.5
    og = torch.arange(k, dtype=torch.float32)
    filt_1d = (1 - torch.abs(og - center) / factor).clamp_min(0)
    return filt_1d.unsqueeze(0) * filt_1d.unsqueeze(1)


def init_deconv_like_bilinear(convT: nn.ConvTranspose2d):
    """
    Initialize ConvTranspose2d (stride=2, k=4) to approximate bilinear upsampling.
    """
    if not isinstance(convT, nn.ConvTranspose2d):
        return
    k = convT.kernel_size[0] if isinstance(convT.kernel_size, tuple) else convT.kernel_size
    s = convT.stride[0] if isinstance(convT.stride, tuple) else convT.stride
    if k != 4 or s != 2:
        return
    with torch.no_grad():
        ker = _bilinear_kernel_2d(k)  # (4,4)
        w = torch.zeros_like(convT.weight.data)
        oc, ic, _, _ = w.shape
        for i in range(oc):
            w[i, i % ic, :, :] = ker
        convT.weight.copy_(w)
        if convT.bias is not None:
            convT.bias.zero_()


# ───────────────────────── Residual blocks ───────────────────────── #

class DownResBlock(nn.Module):
    """
    Residual downsampling block (encoder).
    main: Conv(s=2, bias=False) -> GN -> SiLU -> Conv -> GN -> (scale)
    skip: 1x1 Conv(s=2, bias=True)
    out:  SiLU(main + skip)
    """
    def __init__(self, in_c: int, out_c: int, groups_gn: int, residual_scale: float = 0.1):
        super().__init__()
        self.conv1 = nn.Conv2d(in_c, out_c, kernel_size=3, stride=2, padding=1, bias=False)
        self.gn1 = _gn_for(out_c, groups_gn)
        self.act1 = nn.SiLU()
        self.conv2 = nn.Conv2d(out_c, out_c, kernel_size=3, stride=1, padding=1, bias=False)
        self.gn2 = _gn_for(out_c, groups_gn)
        self.skip = nn.Conv2d(in_c, out_c, kernel_size=1, stride=2, padding=0, bias=True)
        self.act_out = nn.SiLU()
        self.res_scale = float(residual_scale)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h = self.act1(self.gn1(self.conv1(x)))
        h = self.gn2(self.conv2(h))
        h = self.res_scale * h
        s = self.skip(x)
        return self.act_out(h + s)


class ResBlock(nn.Module):
    """
    Same-resolution residual block (decoder).
    main: Conv(bias=False) -> GN -> SiLU -> Conv(bias=False) -> GN -> (scale)
    skip: identity
    out:  SiLU(main + skip)
    """
    def __init__(self, C: int, groups_gn: int, residual_scale: float = 0.1):
        super().__init__()
        self.conv1 = nn.Conv2d(C, C, kernel_size=3, stride=1, padding=1, bias=False)
        self.gn1 = _gn_for(C, groups_gn)
        self.act1 = nn.SiLU()
        self.conv2 = nn.Conv2d(C, C, kernel_size=3, stride=1, padding=1, bias=False)
        self.gn2 = _gn_for(C, groups_gn)
        self.act_out = nn.SiLU()
        self.res_scale = float(residual_scale)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        h = self.act1(self.gn1(self.conv1(x)))
        h = self.gn2(self.conv2(h))
        h = self.res_scale * h
        return self.act_out(x + h)


# ─────────────────────── encoder ─────────────────────── #

class ConvEncoderClassicCC(nn.Module):
    """
    Encoder with optional CoordConv at input and residual downsampling blocks.
    CoordConv here concatenates (x,y) channels to the input (cheap, simple).
    """
    def __init__(
        self,
        in_ch: int,
        z_dim: int,
        base: int,
        levels: int,
        img_size: int,
        groups_gn: int,
        use_coordconv_encoder: bool,
        use_residual: bool = True,
        residual_scale: float = 0.1,
    ):
        super().__init__()
        self.levels = int(levels)
        self.img_size = int(img_size)
        self.use_cc = bool(use_coordconv_encoder)
        self.use_res = bool(use_residual)

        # Optional encoder grid buffer (2,H,W)
        if self.use_cc:
            self.register_buffer("enc_grid", _make_grid(self.img_size, self.img_size), persistent=False)

        in_eff = int(in_ch) + (2 if self.use_cc else 0)
        chs = [in_eff, base, base * 2, base * 4][: self.levels + 1]

        if self.use_res:
            self.blocks = nn.ModuleList([
                DownResBlock(chs[i], chs[i + 1], groups_gn, residual_scale) for i in range(self.levels)
            ])
        else:
            blocks = []
            for i in range(self.levels):
                out_c = chs[i + 1]
                blocks += [
                    nn.Conv2d(chs[i], out_c, 3, stride=2, padding=1, bias=False),
                    _gn_for(out_c, groups_gn),
                    nn.SiLU(),
                    nn.Conv2d(out_c, out_c, 3, 1, 1, bias=False),
                    _gn_for(out_c, groups_gn),
                    nn.SiLU(),
                ]
            self.net = nn.Sequential(*blocks)

        s = max(1, self.img_size // (2 ** self.levels))
        self._spatial = s
        flat_dim = chs[self.levels] * s * s
        self.z_proj = nn.Linear(flat_dim, z_dim)

    def _get_enc_grid(self, x: torch.Tensor, H: int, W: int) -> torch.Tensor:
        if (H, W) != (self.img_size, self.img_size):
            self.register_buffer("enc_grid", _make_grid(H, W), persistent=False)
        return self.enc_grid.to(dtype=x.dtype)

    def feature(self, x: torch.Tensor) -> torch.Tensor:
        h = x
        if self.use_cc:
            N, _, H, W = h.shape
            grid = self._get_enc_grid(h, H, W)  # (2,H,W)
            h = torch.cat([h, grid.unsqueeze(0).expand(N, -1, -1, -1)], dim=1)

        if self.use_res:
            for blk in self.blocks:
                h = blk(h)
        else:
            h = self.net(h)
        return h.reshape(h.size(0), -1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.z_proj(self.feature(x))


# ─────────────────────── decoder ─────────────────────── #

class ConvDecoderClassicCC(nn.Module):
    """
    Decoder with smooth upsampling (default) or deconv (ablation),
    additive CoordConv injection (identity-preserving), and residual blocks.
    Output head is linear by default to avoid sigmoid saturation.
    """
    def __init__(
        self,
        out_ch: int,
        z_dim: int,
        base: int,
        levels: int,
        img_size: int,
        groups_gn: int,
        use_cc_bot: bool,
        use_cc_all: bool,
        upsample_mode: str = "resize_conv",   # 'resize_conv' | 'deconv'
        output_activation: str = "linear",    # 'linear' | 'tanh' | 'sigmoid'
        use_residual: bool = True,
        deconv_bilinear_init: bool = True,
        residual_scale: float = 0.1,
    ):
        super().__init__()
        assert upsample_mode in ("resize_conv", "deconv")
        assert output_activation in ("linear", "tanh", "sigmoid")

        self.levels = int(levels)
        self.img_size = int(img_size)
        self.use_cc_bot = bool(use_cc_bot)
        self.use_cc_all = bool(use_cc_all)
        self.upsample_mode = upsample_mode
        self.output_activation = output_activation
        self.use_res = bool(use_residual)
        self.deconv_bilinear_init = bool(deconv_bilinear_init)

        chs = [base, base * 2, base * 4][: self.levels]
        s = max(1, self.img_size // (2 ** self.levels))
        self.spatial = s  # used to reshape fc output
        self.stem_ch = chs[-1]

        self.fc = nn.Linear(z_dim, self.stem_ch * s * s)

        # Per-stage coord grids (2, Hs, Ws) as buffers & 1x1 proj (2 -> C)
        self.cc_proj = nn.ModuleList()
        self.up_blocks = nn.ModuleList()
        self.res_blocks = nn.ModuleList()

        outs = list(reversed(chs[:-1])) + [chs[0]]
        prev_ch = self.stem_ch

        for i, next_ch in enumerate(outs):
            # Up block
            if self.upsample_mode == "resize_conv":
                up = nn.Sequential(
                    nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False),
                    nn.Conv2d(prev_ch, next_ch, kernel_size=3, padding=1, bias=False),
                    _gn_for(next_ch, groups_gn),
                    nn.SiLU(),
                )
            else:  # 'deconv'
                up = nn.Sequential(
                    nn.ConvTranspose2d(prev_ch, next_ch, kernel_size=4, stride=2, padding=1, bias=True),
                    _gn_for(next_ch, groups_gn),
                    nn.SiLU(),
                )
            self.up_blocks.append(up)

            # CoordConv projector (additive bias), zero-initialized
            inject = (self.use_cc_all or (i == 0 and self.use_cc_bot))
            if inject:
                proj = nn.Conv2d(2, next_ch, kernel_size=1, bias=True)
                nn.init.zeros_(proj.weight)
                nn.init.zeros_(proj.bias)
                self.cc_proj.append(proj)
            else:
                self.cc_proj.append(nn.Identity())

            # Residual same-res conv block
            if self.use_res:
                self.res_blocks.append(ResBlock(next_ch, groups_gn, residual_scale))
            else:
                self.res_blocks.append(nn.Identity())

            # Stage grid buffer
            Hs = s * (2 ** (i + 1))
            self.register_buffer(f"grid_stage_{i}", _make_grid(Hs, Hs), persistent=False)

            prev_ch = next_ch

        self.out = nn.Conv2d(chs[0], out_ch, kernel_size=3, padding=1, bias=True)

        # Optional: initialize deconvs to bilinear
        if self.upsample_mode == "deconv" and self.deconv_bilinear_init:
            for m in self.up_blocks.modules():
                if isinstance(m, nn.ConvTranspose2d):
                    init_deconv_like_bilinear(m)

    def _stage_grid(self, i: int, dtype: torch.dtype) -> torch.Tensor:
        g: torch.Tensor = getattr(self, f"grid_stage_{i}")
        return g.to(dtype=dtype)

    def forward(self, z: torch.Tensor) -> torch.Tensor:
        B = z.size(0)
        h = self.fc(z).reshape(B, self.stem_ch, self.spatial, self.spatial)

        for i in range(self.levels):
            # upsample
            h = self.up_blocks[i](h)

            # additive CoordConv (identity-preserving at init)
            proj = self.cc_proj[i]
            if not isinstance(proj, nn.Identity):
                grid = self._stage_grid(i, h.dtype)           # (2,H,W)
                pos_bias = proj(grid.unsqueeze(0))            # (1,C,H,W)
                h = h + pos_bias                              # broadcast across batch

            # residual same-res block
            h = self.res_blocks[i](h)

        x = self.out(h)

        if self.output_activation == "tanh":
            return torch.tanh(x)
        elif self.output_activation == "sigmoid":
            return torch.sigmoid(x)
        else:
            return x  # linear (recommended)


# ─────────────────────────── top-level AE ─────────────────────────── #

class ResAutoEncoderCoordConv(nn.Module):
    """
    AE with:
      • Encoder: optional CoordConv on input; residual downsampling
      • Decoder: smooth upsampling + residual blocks; additive CoordConv
      • Upsample: 'resize_conv' (default) or 'deconv' (bilinear-initialized)
      • Output: linear (default), or tanh/sigmoid if desired
    """
    def __init__(self, cfg):
        super().__init__()
        # tolerate cfg or cfg.model
        m = cfg if hasattr(cfg, "in_channels") else cfg.model

        in_ch = int(getattr(m, "in_channels"))
        out_ch = int(getattr(m, "out_channels"))
        z_dim = int(getattr(m, "latent_dim"))
        base = int(getattr(m, "base_channels", 32))
        levels = int(getattr(m, "num_down_levels", 3))
        img_size = int(getattr(m, "image_size", 32))
        groups_gn = int(getattr(m, "groups_gn", 8))

        use_cc_enc = bool(getattr(m, "use_coordconv_encoder", False))
        use_cc_bot = bool(getattr(m, "use_coordconv_decoder_bottleneck", True))
        use_cc_all = bool(getattr(m, "use_coordconv_decoder_all_levels", False))

        res_scale = float(getattr(m, "residual_scale", 0.1))
        res_enc = bool(getattr(m, "residual_encoder", True))
        res_dec = bool(getattr(m, "residual_decoder", True))

        upsample_mode = str(getattr(m, "decoder_upsample_mode", "resize_conv"))
        out_act = str(getattr(m, "output_activation", "linear"))
        deconv_bilin_init = bool(getattr(m, "deconv_bilinear_init", True))

        self.encoder = ConvEncoderClassicCC(
            in_ch=in_ch,
            z_dim=z_dim,
            base=base,
            levels=levels,
            img_size=img_size,
            groups_gn=groups_gn,
            use_coordconv_encoder=use_cc_enc,
            use_residual=res_enc,
            residual_scale=res_scale,
        )
        self.decoder = ConvDecoderClassicCC(
            out_ch=out_ch,
            z_dim=z_dim,
            base=base,
            levels=levels,
            img_size=img_size,
            groups_gn=groups_gn,
            use_cc_bot=use_cc_bot,
            use_cc_all=use_cc_all,
            upsample_mode=upsample_mode,
            output_activation=out_act,
            use_residual=res_dec,
            deconv_bilinear_init=deconv_bilin_init,
            residual_scale=res_scale,
        )

        # latent norm buffers for your callbacks
        self.z_dim = z_dim
        self.register_buffer("latent_norm_mean", torch.zeros(self.z_dim))
        self.register_buffer("latent_norm_std", torch.ones(self.z_dim))

        # VAE compatibility slots (unused by default)
        self.register_parameter("prior_logvar", None)
        self._last_mu = None
        self._last_logvar = None
        self._last_z = None

    # parity with your previous API
    def set_latent_normalization(self, mean: torch.Tensor, std: torch.Tensor, eps: float = 1e-6):
        mean = mean.detach().to(self.latent_norm_mean.device, dtype=self.latent_norm_mean.dtype)
        std = std.detach().to(self.latent_norm_std.device, dtype=self.latent_norm_std.dtype).clamp_min(eps)
        if mean.numel() == 1:
            mean = mean.expand_as(self.latent_norm_mean)
        if std.numel() == 1:
            std = std.expand_as(self.latent_norm_std)
        self.latent_norm_mean.copy_(mean)
        self.latent_norm_std.copy_(std)

    def normalize_latent(self, z: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
        return (z - self.latent_norm_mean) / self.latent_norm_std.clamp_min(eps)

    def denormalize_latent(self, z_hat: torch.Tensor) -> torch.Tensor:
        return z_hat * self.latent_norm_std + self.latent_norm_mean

    def encode(self, x: torch.Tensor) -> torch.Tensor:
        return self.encoder(x)

    def decode(self, z: torch.Tensor) -> torch.Tensor:
        return self.decoder(z)

    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        z = self.encode(x)
        x_hat = self.decode(z)
        return x_hat, z

    def print_model_summary(self):
        total = sum(p.numel() for p in self.parameters())
        trainable = sum(p.numel() for p in self.parameters() if p.requires_grad)
        print(f"[AE] Total params: {total:,} (trainable: {trainable:,})")
