...
def get_config():
    ...
    cfg.experiment    = "analytic_manifold/flat_torus/autoencoder_iso_curv_coordconv_decoderbottleneck"
    ...
    data.dataset_path  = "datasets/cache/analytic_T2_32x32_rgb.pt"  # or T^2 path
    ...
    data.manifold      = "torus"        # change to "torus" for the flat torus
    ...
    # local isometry regs
    loss.enc_iso_weight   = 0.01
    loss.dec_iso_weight   = 0.01
    loss.num_v            = 1

    # --- MECAE (extrinsic) ---
    loss.curvature_weight = 0.002 #0.1
    loss.curvature = curv = ml_collections.ConfigDict()
    curv.mode              = "mecae"
    curv.target            = "both"        # "encoder" | "decoder" | "both"
    curv.reg_lambda        = 1e-4
    curv.K_v               = 1
    curv.K_w               = 3             # latent dim m
    ...
    lat.lam_metric       = 1e-3
    lat.t_value          = 0.05
    lat.diffusion_config = "configs/analytic_manifold/flat_torus/latent_config.py"
