# configs/rendered_so_dataset/autoencoder_s2_axisangle_64x64_gray_SCORE_METRIC.py
...
def get_config():
    ...
    cfg.experiment    = "autoencoder_latent_score_metric_smoothness"
    ...
    # Whitney-friendly latent dimension
    model.latent_dim       = 4
    ...
    # reconstruction only (+ score-metric smoothness)
    loss.reconstruction = "mse"
    loss.beta_kl        = 0.0

    # turn OFF isometry regs
    loss.enc_iso_weight = 0.0
    loss.dec_iso_weight = 0.0
    loss.num_v          = 1

    # turn OFF curvature regs
    ...
    # turn OFF decoder metric smoothness
    ...
    # NEW: latent score-metric smoothness (ON)
    loss.score_metric_smooth_weight = 1e-4
    loss.score_metric_smoothness = sms = ml_collections.ConfigDict()
    sms.every_n_steps = 3
    sms.B_score       = 24
    sms.delta_std     = 1e-2
    sms.eps_metric    = 1e-3
    sms.use_latent_ema_for_metric = True
    sms.start_after_steps = 500

    # ---------------- geometry (used to decide latent diffusion is enabled + t*) ----------------
    ...
    # Latent-space geometry (ENABLED: trains latent diffusion online; also defines t* for metric)
    geom.latent = lat = ml_collections.ConfigDict()
    lat.enabled          = True
    lat.metric_type      = "jacobian"
    lat.lam_metric       = 1e-3
    lat.t_value          = 0.05  # <-- t* used by score-metric smoothness + later geodesics
