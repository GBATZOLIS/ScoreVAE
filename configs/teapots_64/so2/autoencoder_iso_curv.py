...
def get_config():
    ...
    cfg.experiment    = "teapots_64/so2/iso_curv_autoencoder_v2"
    ...
    data.elev_step     = None
    data.roll_step     = None
    data.render_batch  = 4                   # render how many images at once (reduce if OOM)
    ...
    model.base_channels   = 64                # a touch wider for teapots
    ...
    loss.enc_iso_weight   = 0.002
    loss.dec_iso_weight   = 0.002
    loss.num_v            = 1

    loss.curvature_weight = 0.002
    loss.curvature = curv = ml_collections.ConfigDict()
    curv.mode              = "mecae"
    curv.target            = "both"
    curv.reg_lambda        = 1e-4
    curv.K_v               = 1
    curv.K_w               = model.latent_dim
