...
from data_geometry.utils.diffusion_utils import (
    get_score_fn as dg_get_score_fn, 
    get_denoiser_fn as dg_get_denoiser_fn
)
...
def finalize_distributed(device=None, ddp_is_active=lambda: dist.is_available() and dist.is_initialized(), ddp_rank=lambda: dist.get_rank() if dist.is_available() and dist.is_initialized() else 0):
    try:
        if ddp_is_active():
            # Make sure all CUDA work is done before tearing down NCCL
            try:
                if device is not None and torch.cuda.is_available():
                    torch.cuda.synchronize(device)
            except Exception:
                pass
            # Ensure all ranks have finished their work
            try:
                dist.barrier()
            except Exception:
                pass
        ...
        print(f"[rank{ddp_rank()}] finalize_distributed warning: {e}")
...
# Post traing for latent diffusion model
...
def post_train_latent(
    *,
    ...
    post_grad_clip   = float(getattr(post_cfg, "grad_clip", getattr(lat_cfg.optim, "grad_clip", 1.0)))
    ...
    post_gen_every = int(getattr(post_cfg, "gen_every", 1))  # default: every epoch
    ...
    if lat_cfg is not None:
        import copy
        lat_cfg_post = copy.deepcopy(lat_cfg)
    ...
    if ddp_rank() == 0:
        print(f"[Latent Post-Train] starting {post_steps} iterations. "
              f"lr={getattr(lat_cfg_post.optim, 'lr', None) if lat_cfg_post else 'n/a'}, "
              f"warmup={post_warmup}, clip={post_grad_clip}")
    ...
    def _run_validation(epoch_idx: int) -> float:
        ...
        if use_tqdm:
            ...
            vbar = tqdm(total=total, leave=False, desc=f"Val (epoch {epoch_idx+1})")
    ...
    for post_epoch in range(max_epochs):
        ...
        if use_tqdm:
            ...
            pbar = tqdm(total=steps_per_epoch, leave=False, desc=desc) if steps_per_epoch is not None else tqdm(leave=False, desc=desc)
...
def train(cfg):
    ...
    for epoch in range(start_epoch, cfg.training.epochs):
        ...
        for data in it:
            ...
            if ddp_rank() == 0 and isinstance(it, tqdm):
    ...
    # Continue training the latent diffusion model AFTER AE finishes, so it adapts properly
    # to the converged latent distribution. Defaults: 20_000 steps with a fresh
    # optimizer/scheduler (configurable via lat_cfg.post_train.*).
    if latent_model is not None:
        post_stats = post_train_latent(
            ...
            train_sampler=train_sampler,           # if you use DistributedSampler
            lat_cfg=lat_cfg,
            latent_sde=latent_sde,
            writer=writer,
            device=device,
...
def main():
    P = argparse.ArgumentParser("AutoEncoder trainer with curvature regularization (DDP + latent post-train)")
    P.add_argument("--config", required=True, help="Path to the AE configuration (.py).")
    args = P.parse_args()

    cfg = load_config(args.config)

    # NEW: Optional multi-GPU selection field; keeps single-GPU backward-compat
