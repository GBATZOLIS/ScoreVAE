    ...
    *,
    ...
    if entropic_profile_enabled and lat_model is not None and lat_sde is not None:
        ...
        if bool(geo.get("use_entropic_schedule", False)):
            source = str(geo.get("entropic_source", "mmse")).lower()
            if source == "score":
    ...
    if geo_config is not None and lat_model is not None and lat_sde is not None:
        ...
        imgs, rots = [], []
        for j in perm_local:
            x_j, r_j = base_ds[idx_pool[j]]
            imgs.append(x_j)
            rots.append(r_j)

        X = torch.stack(imgs, dim=0).to(device)
        Rsel = torch.stack(rots, dim=0)
        ...
        if B > 0:
            ...
            try:
                if hasattr(base_ds, "compute_geodesic"):
                    ...
                    pred_imgs = []
                    for b in range(B):
                        z_traj_b = torch.stack([path_z_denorm[k][b] for k in range(T)], dim=0).to(device)
                        with torch.no_grad():
                            dec_b = model.decode(z_traj_b)
                    ...
                    gt_stack = base_ds.compute_geodesic(P, Q, t_lin).detach().cpu()
    ...
    if do_sample and (lat_model is not None and lat_sde is not None):
        ...
        z_hat = Algorithm1(lat_sde, int(sample_steps), score_fn, z_shape, device, y=None)
...
def main():
    ...
    # geodesics
    p.add_argument("--geo-config", type=str, default=None,
